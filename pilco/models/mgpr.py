...
class MGPR(gpflow.Module):
    ...
    def create_models(self, data):
        ...
        for i in range(self.num_outputs):
            ...
            kern.lengthscales.prior = tfd.Gamma(to_default_float(1.1),
                                                to_default_float(1 / 10.0))  # priors have to be included before
            kern.variance.prior = tfd.Gamma(to_default_float(1.5),
                                            to_default_float(1 / 2.0))  # before the model gets compiled
    ...
    def optimize(self, restarts=1):
        if len(self.optimizers) == 0:  # This is the first call to optimize();
            ...
            for model in self.models:
                ...
                #         print("Shape: ", v.shape)
                #         print(v)
                # print('training_loss: {}'.format(model.training_loss))
                # print('trainable_variables: {}'.format(model.trainable_variables))

                # print(optimizer.minimize(model.training_loss, model.trainable_variables))

                optimizer.minimize(model.training_loss, model.trainable_variables)
                print('model GP{} is right'.format(i))
                i += 1
            ...
            for model, optimizer in zip(self.models, self.optimizers):
                ...
                # print(optimizer.minimize(model.training_loss, model.trainable_variables, compile=True))
                optimizer.minimize(model.training_loss, model.trainable_variables)
                print('model GP{} is right'.format(i))
                i += 1
